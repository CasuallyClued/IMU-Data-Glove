 ...
 *  modified by Kamal Gamir-Shahin
 *
...
void Adafruit_LIS3DH::read()
{
  ...
  z_g = (float)z / divider;
  // Serial.begin(115200);
  // Serial.printf("x digital: %e,    y digital: %e,     z digital: %e\n", x_g, y_g, z_g);
}
...
/**
 * @brief Gets the acceleration values in g and sets the G_Values struct
 * 
 * @param g_values 3D acceleration vector struct to be set
 */
void Adafruit_LIS3DH::get_g_values(G_Values *g_values)
{
  read(); // Get the value (read the accelerometer)

  g_values->x = x_g * SENSORS_GRAVITY_STANDARD; // Set x-axis vector element
  g_values->y = y_g * SENSORS_GRAVITY_STANDARD; // Set y-axis vector element
  g_values->z = z_g * SENSORS_GRAVITY_STANDARD; // Set z-axis vector element
}