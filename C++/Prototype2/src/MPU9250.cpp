...
void MPU9250::magCalMPU9250(float *bias_dest, float *scale_dest)
{
  ...
  bias_dest[0] = (float)mag_bias[0] * mRes * magCalibration[0];
  bias_dest[1] = (float)mag_bias[1] * mRes * magCalibration[1];
  bias_dest[2] = (float)mag_bias[2] * mRes * magCalibration[2];
...
void MPU9250::do_all()
{
  ...
  Serial.print(9.80665 * ax);
  Serial.print(" g ");
  Serial.print("Y-acceleration: ");
  Serial.print(9.80665 * ay);
  Serial.print(" g ");
  Serial.print("Z-acceleration: ");
  Serial.print(9.80665 * az);
  Serial.println(" g ");
...
void MPU9250::get_IMUdata()
{
  ...
  {                            // On interrupt, check if data ready interrupt
    ...
    ax = (float)accelCount[0] * 9.80665 * aRes; // - accelBias[0];  // get actual g value, this depends on scale being set
    ay = (float)accelCount[1] * 9.80665 * aRes; // - accelBias[1];
    az = (float)accelCount[2] * 9.80665 * aRes; // - accelBias[2];
...
void MPU9250::init_single(int ADpin)
{
  Wire.begin();
  //  TWBR = 12;  // 400 kbit/sec I2C speed
  // Set up the interrupt pin, its set as active high, push-pull
  ...
  byte c = readByte(ADpin, WHO_AM_I_MPU9250); // Read WHO_AM_I register for MPU-9250
  Serial.print("MPU9250 ");
  Serial.print("I AM ");
  Serial.print(c, HEX);
  Serial.print(" I should be ");
...
void MPU9250::get_IMUdata_single(int ADpin)
{
  // If intPin goes high, all data registers have new data
  if (readByte(ADpin, INT_STATUS) & 0x01)
  {                            // On interrupt, check if data ready interrupt
    readAccelData(accelCount); // Read the x/y/z adc values
    getAres();
...
void MPU9250::do_all_single(int ADpin)
{
  get_IMUdata_single(ADpin);
  // Print acceleration values in milligs!
  Serial.print("X-acceleration: ");
  Serial.print(9.80665 * ax);
  Serial.print(" g ");
...
void MPU9250::get_IMUvalues(IMU_Values *accvals, IMU_Values *angvals, IMU_Values *magvals){
if (readByte(MPU9250_ADDRESS, INT_STATUS) & 0x01)
  {                            // On interrupt, check if data ready interrupt
    readAccelData(accelCount); // Read the x/y/z adc values
    getAres();
    ...
    // Now we'll calculate the accleration value into actual g's
    accvals->x = (float)accelCount[0] * 9.80665 * aRes; // - accelBias[0];  // get actual g value, this depends on scale being set
    accvals->y = (float)accelCount[1] * 9.80665 * aRes; // - accelBias[1];
    accvals->z = (float)accelCount[2] * 9.80665 * aRes; // - accelBias[2];

    readGyroData(gyroCount); // Read the x/y/z adc values
    getGres();

    // Calculate the gyro value into actual degrees per second
    angvals->x = (float)gyroCount[0] * gRes; // get actual gyro value, this depends on scale being set
    angvals->y = (float)gyroCount[1] * gRes;
    angvals->z = (float)gyroCount[2] * gRes;
    ...
    // Calculate the magnetometer values in milliGauss
    // Include factory calibration per data sheet and user environmental corrections
    magvals->x = (float)magCount[0] * mRes * magCalibration[0] - magBias[0]; // get actual magnetometer value, this depends on scale being set
    magvals->y = (float)magCount[1] * mRes * magCalibration[1] - magBias[1];
    magvals->z = (float)magCount[2] * mRes * magCalibration[2] - magBias[2];
  }
}